...
exports.answerChatQuestion = exports.summarizeTranscript = exports.transcribeAudio = void 0;
...
const transcribeAudio = async (filePath, agenda, meetingType) => {
    const openai = getOpenAIClient();
    if (openai && fs_1.default.existsSync(filePath)) {
        try {
...
const summarizeTranscript = async (transcript, agenda) => {
    const openai = getOpenAIClient();
    if (openai) {
        try {
...
const answerChatQuestion = async (transcript, question) => {
    const openai = getOpenAIClient();
    if (openai) {
        try {
...
exports.answerChatQuestion = answerChatQuestion;
// --- HELPER GENERATORS FOR SMART MOCK FALLBACKS ---
const generateSmartMockTranscript = (agenda, meetingType) => {
...
const generateSmartMockSummary = (agenda) => {
    const isJalJeevan = agenda.toLowerCase().includes('water') || agenda.toLowerCase().includes('jal');
    const isMgnrega = agenda.toLowerCase().includes('mgnrega') || agenda.toLowerCase().includes('road');
    const isAwas = agenda.toLowerCase().includes('awas') || agenda.toLowerCase().includes('hous');
    ...
    return {
        executiveSummary: `The Panchayat meeting successfully addressed critical development priorities outlined in the agenda. Focused discussions led to key resolutions regarding ${isMgnrega ? 'road repair infrastructure, ' : ''}${isJalJeevan ? 'drinking water supply expansion under the Jal Jeevan Mission, ' : ''}and welfare beneficiaries. Action plans were established with specific officers assigned to ensure timely execution.`,
        keyDiscussionPoints,
        decisionsTaken,
        schemesMentioned,
        budgetDiscussions,
